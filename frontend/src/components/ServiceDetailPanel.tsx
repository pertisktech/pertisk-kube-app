    ...
    <aside className="fixed top-0 right-0 z-[100] h-screen w-[420px] max-w-[94vw] bg-surface-elevated border-l border-border shadow-2xl">
      <div className="h-full flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="text-lg font-semibold text-text">Service Info</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          <section className="min-w-0 bg-surface border border-border rounded-lg p-4">
            <div className="space-y-3">
              <div>
                ...
                <p className="text-primary font-medium break-all">{service.name}</p>
              </div>
              <div>
                <p className="text-text-secondary">Namespace</p>
                <p className="text-text break-all">{service.namespace}</p>
              </div>
              <div>
                <p className="text-text-secondary">Type</p>
                <p className="text-text">{service.service_type || '-'}</p>
              </div>
              <div>
                <p className="text-text-secondary">Cluster IP</p>
                <p className="text-text font-mono">{service.cluster_ip || '-'}</p>
              </div>
              <div>
                <p className="text-text-secondary">External IP</p>
                <p className="text-text font-mono">{service.external_ip || '-'}</p>
              </div>
              <div>
                <p className="text-text-secondary">Ports</p>
                <p className="text-text font-mono break-all">{service.ports || '-'}</p>
              </div>
              <div>
                <p className="text-text-secondary">Age</p>
                <p className="text-text">{timeAgo(service.age)}</p>
              </div>
            </div>
          </section>
        </div>
