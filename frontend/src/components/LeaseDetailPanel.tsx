  ...
  return (
    <ResizablePanel>
      <div className="h-full flex flex-col">
        <DetailPanelHeader title="Lease Info" onClose={onClose}>
          <div className="flex gap-2">
            <div className="group relative">
              <button type="button" onClick={() => onOpenYamlEditor?.(lease)} className="p-2 rounded-md border border-[var(--color-primary)] text-[var(--color-primary)] hover:bg-[var(--color-primary)]/10 transition-colors" aria-label="Edit lease YAML"><Pencil size={12} /></button>
            ...
            <div className="group relative">
              <button type="button" onClick={() => onDelete?.(lease.namespace, lease.name)} className="p-2 rounded-md border border-[var(--color-icon-danger)] text-[var(--color-icon-danger)] hover:bg-[var(--color-icon-danger)]/10 transition-colors" aria-label="Delete lease"><Trash2 size={12} /></button>
          ...
          <section className="min-w-0 bg-surface border border-border rounded-lg p-4">
            <div className="space-y-3">
              <div>
                ...
                <p className="text-primary font-medium break-all">{lease.name}</p>
              </div>
              <div>
                <p className="text-text-secondary">Namespace</p>
                <p className="text-text break-all">{lease.namespace}</p>
              </div>
              <div>
                <p className="text-text-secondary">Holder Identity</p>
                <p className="text-text break-all">{lease.holder_identity}</p>
              </div>
              <div>
                <p className="text-text-secondary">Lease Duration</p>
                <p className="text-text">{lease.lease_duration_seconds}s</p>
              </div>
              <div>
                <p className="text-text-secondary">Age</p>
                <p className="text-text">{timeAgo(lease.age)}</p>
              </div>
            </div>
          </section>
        </div>
