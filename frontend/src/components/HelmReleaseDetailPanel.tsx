...
const getLifecycleDescription = (status: string): string => {
  ...
  switch (s) {
    case 'deployed':       return 'Release is active and running.';
    case 'pending-install': return 'Install is in progress.';
    case 'pending-upgrade': return 'Upgrade is in progress.';
    case 'pending-rollback': return 'Rollback is in progress.';
    case 'uninstalling':   return 'Uninstall is in progress (helm uninstall).';
    case 'uninstalled':    return 'Release has been uninstalled.';
    case 'failed':         return 'Release install, upgrade, or rollback failed.';
    case 'superseded':     return 'Superseded by a newer revision.';
    default:               return 'Release lifecycle state.';
  }
};
  ...
  >
    ...
    <div className="space-y-2">
      {resourcesLoading ? (
        ...
        <p className="text-sm" style={{ color: 'var(--color-muted)' }}>No resources found.</p>
      ) : (
        <div className="rounded-lg border border-border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {resources.map((r, idx) => (
                <tr key={idx} className="border-b border-border last:border-b-0 hover:opacity-90">
