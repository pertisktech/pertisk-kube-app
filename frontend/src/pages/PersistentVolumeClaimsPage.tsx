...
export const PersistentVolumeClaimsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<PersistentVolumeClaim[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/persistentvolumeclaims/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: PersistentVolumeClaim) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/persistentvolumeclaims/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
      if (!res.ok) { const msg = await res.text(); throw new Error(msg || `Failed to save (${res.status})`); }
      setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: 'PVC saved successfully' }));
