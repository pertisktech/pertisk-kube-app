...
export const EndpointsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<Endpoint[]>([]);
  ...
  const activeYamlItem = useMemo(() => activeYamlTabKey ? yamlTabs.find((t) => getKey(t) === activeYamlTabKey) ?? null : null, [yamlTabs, activeYamlTabKey]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/endpoints/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: Endpoint) => {
  ...
  const handleCloseYamlTab = (tabKey: string) => {
    setYamlTabs((p) => { const next = p.filter((t) => getKey(t) !== tabKey); if (activeYamlTabKey === tabKey) setActiveYamlTabKey(next[next.length - 1] ? getKey(next[next.length - 1]) : null); return next; });
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/endpoints/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
      if (!res.ok) { const msg = await res.text(); throw new Error(msg || `Failed to save (${res.status})`); }
      setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: 'Endpoint saved successfully' }));
              ...
              <div className="bg-surface border border-border rounded-t-lg shadow-2xl p-3 pt-5 space-y-2 overflow-auto relative" style={{ height: yamlDrawerHeightPx ? `${yamlDrawerHeightPx}px` : 'clamp(220px, 45vh, 620px)' }}>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 min-w-0">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 min-w-0">
                    {yamlTabs.map((tab) => { const tabKey = getKey(tab); const isActive = tabKey === activeYamlTabKey; return (
