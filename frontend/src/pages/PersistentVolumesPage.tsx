...
export const PersistentVolumesPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<PersistentVolume[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/persistentvolumes/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: PersistentVolume) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/persistentvolumes/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
      if (!res.ok) { const msg = await res.text(); throw new Error(msg || `Failed to save (${res.status})`); }
      setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: 'PV saved successfully' }));
