...
export const ConfigMapsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<ConfigMap[]>([]);
  ...
  const getConfigMapKey = (configMap: ConfigMap) => `${configMap.namespace}/${configMap.name}`;

  const activeYamlConfigMap = useMemo(() => {
    if (!activeYamlTabKey) {
      return null;
    }
    return yamlTabs.find((cm) => getConfigMapKey(cm) === activeYamlTabKey) ?? null;
  ...
  useEffect(() => {
    if (!activeYamlConfigMap || !activeYamlTabKey) {
      return;
    }
    ...
    const loadYaml = async () => {
      ...
      try {
        const response = await fetch(
          `/api/configmaps/${encodeURIComponent(activeYamlConfigMap.namespace)}/${encodeURIComponent(activeYamlConfigMap.name)}/yaml`,
          {
            method: 'GET',
        ...
        if (!response.ok) {
          throw new Error(`Failed to load configmap YAML (${response.status})`);
        }

        const yaml = await response.text();
        const sanitizedYaml = sanitizeConfigMapYamlForEdit(yaml);
        setYamlContentsByTab((previous) => ({ ...previous, [activeYamlTabKey]: sanitizedYaml }));
      } catch (error) {
        if ((error as Error).name !== 'AbortError') {
          setYamlErrorByTab((previous) => ({
            ...previous,
            [activeYamlTabKey]: (error as Error).message || 'Failed to load configmap YAML',
          }));
        }
      } finally {
  ...
  }, [activeYamlConfigMap?.namespace, activeYamlConfigMap?.name, activeYamlTabKey, yamlContentsByTab]);
  ...
  const handleOpenYamlEditorFromPanel = (configMap: ConfigMap) => {
    const key = getConfigMapKey(configMap);

    setYamlTabs((previousTabs) => {
      if (previousTabs.some((tab) => getConfigMapKey(tab) === key)) {
        return previousTabs;
      }
      return [...previousTabs, configMap];
    });
  ...
  const handleCloseYamlTab = (tabKey: string) => {
    setYamlTabs((previousTabs) => {
      const nextTabs = previousTabs.filter((tab) => getConfigMapKey(tab) !== tabKey);

      if (activeYamlTabKey === tabKey) {
        const fallbackTab = nextTabs[nextTabs.length - 1];
        setActiveYamlTabKey(fallbackTab ? getConfigMapKey(fallbackTab) : null);
      }
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || `Failed to save configmap YAML (${response.status})`);
      }

      setYamlSuccessByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: 'ConfigMap updated successfully',
      }));
    } catch (error) {
      setYamlErrorByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: (error as Error).message || 'Failed to save configmap YAML',
      }));
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {yamlTabs.length > 0 && yamlDrawerVisible && activeYamlConfigMap && activeYamlTabKey && (
          <section className="fixed bottom-0 left-0 md:left-[var(--layout-sidebar-width,16rem)] right-0 z-[96]">
            <div className="px-6">
              <div
              ...
              >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 min-w-0">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 min-w-0">
                    {yamlTabs.map((tab) => {
                      const tabKey = getConfigMapKey(tab);
                ...
                <>
                  ...
                  <div className="w-full h-[calc(100%-90px)] min-h-[180px] rounded-md border border-border bg-surface-elevated overflow-hidden">
                    <AceEditor
                      ...
                      name={`configmap-yaml-editor-${activeYamlTabKey}`}
