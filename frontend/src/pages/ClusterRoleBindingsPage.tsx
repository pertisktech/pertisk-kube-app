...
import { useClusterRoleBindings, deleteClusterRoleBinding } from '../hooks/useKubernetes';
...
type ClusterRoleBindingsSortKey = 'name' | 'age';
...
export const ClusterRoleBindingsPage = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<ClusterRoleBinding | null>(null);
  const [panelOpen, setPanelOpen] = useState(false);
  const [yamlTabs, setYamlTabs] = useState<ClusterRoleBinding[]>([]);
  ...
  const [sortState, setSortState] = useState<{ key: ClusterRoleBindingsSortKey; direction: 'asc' | 'desc' }>({ key: 'age', direction: 'desc' });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/clusterrolebindings/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: ClusterRoleBinding) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/clusterrolebindings/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try { await Promise.all(confirmDelete.keys.map((name) => deleteClusterRoleBinding(name))); setSelectedRows([]); setConfirmDelete(null); }
    finally { setIsDeleting(false); }
  };

  const columns = [
    { header: 'Name', accessor: 'name' as const, width: '35%', sortable: true, sortKey: 'name' },
    { header: 'Role', accessor: 'role' as const, width: '30%' },
    { header: 'Subjects', accessor: 'subjects' as const, width: '15%' },
    { header: 'Age', accessor: (r: ClusterRoleBinding) => timeAgo(r.age), width: '20%', sortable: true, sortKey: 'age' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2" style={{ paddingBottom: yamlTabs.length > 0 ? (yamlDrawerVisible ? yamlDrawerHeightPx ?? 420 : 84) : 0 }}>
        <DataTable
          ...
          sortState={sortState} onSortChange={(s) => setSortState(s as { key: ClusterRoleBindingsSortKey; direction: 'asc' | 'desc' })}
      ...
      {panelOpen && selectedItem && (
        <>
          ...
          <ClusterRoleBindingDetailPanel clusterRoleBinding={selectedItem} onClose={() => setPanelOpen(false)} onOpenYamlEditor={handleOpenYamlEditorFromPanel} onDelete={handleDeleteSingle} />
        </>
      )}
