...
import { useIngressClasses, deleteIngressClass } from '../hooks/useKubernetes';
...
export const IngressClassesPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<IngressClass[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/ingressclasses/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: IngressClass) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/ingressclasses/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
      if (!res.ok) { const msg = await res.text(); throw new Error(msg || `Failed to save (${res.status})`); }
      setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: 'Ingress Class saved successfully' }));
