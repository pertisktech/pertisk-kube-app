...
export const RoleBindingsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<RoleBinding[]>([]);
  ...
  const [sortState, setSortState] = useState<{ key: RoleBindingsSortKey; direction: 'asc' | 'desc' }>({ key: 'age', direction: 'desc' });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/rolebindings/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: RoleBinding) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/rolebindings/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await Promise.all(confirmDelete.keys.map((key) => { const [ns, ...rest] = key.split('/'); return deleteRoleBinding(ns, rest.join('/')); }));
      setSelectedRows([]); setConfirmDelete(null);
    } finally { setIsDeleting(false); }
  };

  const columns = [
    { header: 'Name', accessor: 'name' as const, width: '25%', sortable: true, sortKey: 'name' },
    { header: 'Namespace', accessor: 'namespace' as const, width: '20%', sortable: true, sortKey: 'namespace' },
    { header: 'Role', accessor: 'role' as const, width: '25%' },
    { header: 'Subjects', accessor: 'subjects' as const, width: '13%' },
    { header: 'Age', accessor: (r: RoleBinding) => timeAgo(r.age), width: '17%', sortable: true, sortKey: 'age' },
  ];
    ...
    <div className="space-y-6">
      ...
      {panelOpen && selectedItem && (
        <>
          ...
          <RoleBindingDetailPanel roleBinding={selectedItem} onClose={() => setPanelOpen(false)} onOpenYamlEditor={handleOpenYamlEditorFromPanel} onDelete={handleDeleteSingle} />
        </>
      )}
