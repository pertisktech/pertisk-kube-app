...
export const StatefulSetsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<StatefulSet[]>([]);
  ...
  const getStatefulSetKey = (statefulSet: StatefulSet) => `${statefulSet.namespace}/${statefulSet.name}`;

  const activeYamlStatefulSet = useMemo(() => {
    if (!activeYamlTabKey) {
      return null;
    }
    return yamlTabs.find((statefulSet) => getStatefulSetKey(statefulSet) === activeYamlTabKey) ?? null;
  ...
  useEffect(() => {
    if (!activeYamlStatefulSet || !activeYamlTabKey) {
      return;
    }
    ...
    const loadYaml = async () => {
      ...
      try {
        const response = await fetch(
          `/api/statefulsets/${encodeURIComponent(activeYamlStatefulSet.namespace)}/${encodeURIComponent(activeYamlStatefulSet.name)}/yaml`,
          {
            method: 'GET',
        ...
        if (!response.ok) {
          throw new Error(`Failed to load statefulset YAML (${response.status})`);
        }

        const yaml = await response.text();
        const sanitizedYaml = sanitizeStatefulSetYamlForEdit(yaml);
        setYamlContentsByTab((previous) => ({ ...previous, [activeYamlTabKey]: sanitizedYaml }));
      } catch (error) {
        if ((error as Error).name !== 'AbortError') {
          setYamlErrorByTab((previous) => ({
            ...previous,
            [activeYamlTabKey]: (error as Error).message || 'Failed to load statefulset YAML',
          }));
        }
      } finally {
  ...
  }, [activeYamlStatefulSet?.namespace, activeYamlStatefulSet?.name, activeYamlTabKey, yamlContentsByTab]);
  ...
  const handleOpenYamlEditorFromPanel = (statefulSet: StatefulSet) => {
    const key = getStatefulSetKey(statefulSet);

    setYamlTabs((previousTabs) => {
      if (previousTabs.some((tab) => getStatefulSetKey(tab) === key)) {
        return previousTabs;
      }
      return [...previousTabs, statefulSet];
    });
  ...
  const handleCloseYamlTab = (tabKey: string) => {
    setYamlTabs((previousTabs) => {
      const nextTabs = previousTabs.filter((tab) => getStatefulSetKey(tab) !== tabKey);

      if (activeYamlTabKey === tabKey) {
        const fallbackTab = nextTabs[nextTabs.length - 1];
        setActiveYamlTabKey(fallbackTab ? getStatefulSetKey(fallbackTab) : null);
      }
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || `Failed to save statefulset YAML (${response.status})`);
      }

      setYamlSuccessByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: 'StatefulSet YAML saved successfully',
      }));
    } catch (error) {
      setYamlErrorByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: (error as Error).message || 'Failed to save statefulset YAML',
      }));
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {yamlTabs.length > 0 && yamlDrawerVisible && activeYamlStatefulSet && activeYamlTabKey && (
          <section className="fixed bottom-0 left-0 md:left-[var(--layout-sidebar-width,16rem)] right-0 z-[96]">
            <div className="px-6">
              <div
              ...
              >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 min-w-0">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 min-w-0">
                    {yamlTabs.map((tab) => {
                      const tabKey = getStatefulSetKey(tab);
                ...
                <>
                  ...
                  <div className="w-full h-[calc(100%-90px)] min-h-[180px] rounded-md border border-border bg-surface-elevated overflow-hidden">
                    <AceEditor
                      ...
                      name={`statefulset-yaml-editor-${activeYamlTabKey}`}
