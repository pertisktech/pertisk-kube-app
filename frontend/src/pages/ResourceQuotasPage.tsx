...
export const ResourceQuotasPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<ResourceQuota[]>([]);
  ...
  const getResourceQuotaKey = (resourceQuota: ResourceQuota) => `${resourceQuota.namespace}/${resourceQuota.name}`;
  
  const activeYamlResourceQuota = useMemo(() => {
    if (!activeYamlTabKey) {
      return null;
    }
    return yamlTabs.find((rq) => getResourceQuotaKey(rq) === activeYamlTabKey) ?? null;
  ...
  useEffect(() => {
    if (!activeYamlResourceQuota || !activeYamlTabKey) {
      return;
    }
    ...
    const loadYaml = async () => {
      ...
      try {
        const response = await fetch(
          `/api/resourcequotas/${encodeURIComponent(activeYamlResourceQuota.namespace)}/${encodeURIComponent(activeYamlResourceQuota.name)}/yaml`,
          {
            method: 'GET',
        ...
        if (!response.ok) {
          throw new Error(`Failed to load resourcequota YAML (${response.status})`);
        }

        const yaml = await response.text();
        const sanitizedYaml = sanitizeResourceQuotaYamlForEdit(yaml);
        setYamlContentsByTab((previous) => ({ ...previous, [activeYamlTabKey]: sanitizedYaml }));
      } catch (error) {
        if ((error as Error).name !== 'AbortError') {
          setYamlErrorByTab((previous) => ({
            ...previous,
            [activeYamlTabKey]: (error as Error).message || 'Failed to load resourcequota YAML',
          }));
        }
      } finally {
  ...
  }, [activeYamlResourceQuota?.namespace, activeYamlResourceQuota?.name, activeYamlTabKey, yamlContentsByTab]);
  ...
  const handleOpenYamlEditorFromPanel = (resourceQuota: ResourceQuota) => {
    const key = getResourceQuotaKey(resourceQuota);

    setYamlTabs((previousTabs) => {
      if (previousTabs.some((tab) => getResourceQuotaKey(tab) === key)) {
        return previousTabs;
      }
      return [...previousTabs, resourceQuota];
    });
  ...
  const handleCloseYamlTab = (tabKey: string) => {
    setYamlTabs((previousTabs) => {
      const nextTabs = previousTabs.filter((tab) => getResourceQuotaKey(tab) !== tabKey);

      if (activeYamlTabKey === tabKey) {
        const fallbackTab = nextTabs[nextTabs.length - 1];
        setActiveYamlTabKey(fallbackTab ? getResourceQuotaKey(fallbackTab) : null);
      }
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || `Failed to save resourcequota YAML (${response.status})`);
      }

      setYamlSuccessByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: 'ResourceQuota updated successfully',
      }));
    } catch (error) {
      setYamlErrorByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: (error as Error).message || 'Failed to save resourcequota YAML',
      }));
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {yamlTabs.length > 0 && yamlDrawerVisible && activeYamlResourceQuota && activeYamlTabKey && (
          <section className="fixed bottom-0 left-0 md:left-[var(--layout-sidebar-width,16rem)] right-0 z-[96]">
            <div className="px-6">
              <div
              ...
              >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 min-w-0">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 min-w-0">
                    {yamlTabs.map((tab) => {
                      const tabKey = getResourceQuotaKey(tab);
                ...
                <>
                  ...
                  <div className="w-full h-[calc(100%-90px)] min-h-[180px] rounded-md border border-border bg-surface-elevated overflow-hidden">
                    <AceEditor
                      ...
                      name={`resourcequota-yaml-editor-${activeYamlTabKey}`}
