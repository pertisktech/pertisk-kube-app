...
export const ServiceAccountsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<ServiceAccount[]>([]);
  ...
  const [sortState, setSortState] = useState<{ key: ServiceAccountsSortKey; direction: 'asc' | 'desc' }>({ key: 'age', direction: 'desc' });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/serviceaccounts/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: ServiceAccount) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/serviceaccounts/${encodeURIComponent(activeYamlItem.namespace)}/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await Promise.all(confirmDelete.keys.map((key) => { const [ns, ...rest] = key.split('/'); return deleteServiceAccount(ns, rest.join('/')); }));
      setSelectedRows([]); setConfirmDelete(null);
    } finally { setIsDeleting(false); }
  };

  const columns = [
    { header: 'Name', accessor: 'name' as const, width: '30%', sortable: true, sortKey: 'name' },
    { header: 'Namespace', accessor: 'namespace' as const, width: '25%', sortable: true, sortKey: 'namespace' },
    { header: 'Secrets', accessor: 'secrets' as const, width: '20%' },
    { header: 'Age', accessor: (r: ServiceAccount) => timeAgo(r.age), width: '25%', sortable: true, sortKey: 'age' },
  ];
    ...
    <div className="space-y-6">
      ...
      {panelOpen && selectedItem && (
        <>
          ...
          <ServiceAccountDetailPanel serviceAccount={selectedItem} onClose={() => setPanelOpen(false)} onOpenYamlEditor={handleOpenYamlEditorFromPanel} onDelete={handleDeleteSingle} />
        </>
      )}
