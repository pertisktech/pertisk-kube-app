...
export const CronJobsPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<CronJob[]>([]);
  ...
  const getCronJobKey = (cronJob: CronJob) => `${cronJob.namespace}/${cronJob.name}`;

  const activeYamlCronJob = useMemo(() => {
    if (!activeYamlTabKey) {
      return null;
    }
    return yamlTabs.find((cronJob) => getCronJobKey(cronJob) === activeYamlTabKey) ?? null;
  ...
  useEffect(() => {
    if (!activeYamlCronJob || !activeYamlTabKey) {
      return;
    }
    ...
    const loadYaml = async () => {
      ...
      try {
        const response = await fetch(
          `/api/cronjobs/${encodeURIComponent(activeYamlCronJob.namespace)}/${encodeURIComponent(activeYamlCronJob.name)}/yaml`,
          {
            method: 'GET',
        ...
        if (!response.ok) {
          throw new Error(`Failed to load cronjob YAML (${response.status})`);
        }

        const yaml = await response.text();
        const sanitizedYaml = sanitizeCronJobYamlForEdit(yaml);
        setYamlContentsByTab((previous) => ({ ...previous, [activeYamlTabKey]: sanitizedYaml }));
      } catch (error) {
        if ((error as Error).name !== 'AbortError') {
          setYamlErrorByTab((previous) => ({
            ...previous,
            [activeYamlTabKey]: (error as Error).message || 'Failed to load cronjob YAML',
          }));
        }
      } finally {
  ...
  }, [activeYamlCronJob?.namespace, activeYamlCronJob?.name, activeYamlTabKey, yamlContentsByTab]);
  ...
  const handleOpenYamlEditorFromPanel = (cronJob: CronJob) => {
    const key = getCronJobKey(cronJob);

    setYamlTabs((previousTabs) => {
      if (previousTabs.some((tab) => getCronJobKey(tab) === key)) {
        return previousTabs;
      }
      return [...previousTabs, cronJob];
    });
  ...
  const handleCloseYamlTab = (tabKey: string) => {
    setYamlTabs((previousTabs) => {
      const nextTabs = previousTabs.filter((tab) => getCronJobKey(tab) !== tabKey);

      if (activeYamlTabKey === tabKey) {
        const fallbackTab = nextTabs[nextTabs.length - 1];
        setActiveYamlTabKey(fallbackTab ? getCronJobKey(fallbackTab) : null);
      }
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || `Failed to save cronjob YAML (${response.status})`);
      }

      setYamlSuccessByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: 'CronJob YAML saved successfully',
      }));
    } catch (error) {
      setYamlErrorByTab((previous) => ({
        ...previous,
        [activeYamlTabKey]: (error as Error).message || 'Failed to save cronjob YAML',
      }));
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      />
        ...
        {yamlTabs.length > 0 && yamlDrawerVisible && activeYamlCronJob && activeYamlTabKey && (
          <section className="fixed bottom-0 left-0 md:left-[var(--layout-sidebar-width,16rem)] right-0 z-[96]">
            <div className="px-6">
              <div
              ...
              >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 min-w-0">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 min-w-0">
                    {yamlTabs.map((tab) => {
                      const tabKey = getCronJobKey(tab);
                ...
                <>
                  ...
                  <div className="w-full h-[calc(100%-90px)] min-h-[180px] rounded-md border border-border bg-surface-elevated overflow-hidden">
                    <AceEditor
                      ...
                      name={`cronjob-yaml-editor-${activeYamlTabKey}`}
