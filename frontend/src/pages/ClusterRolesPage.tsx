...
export const ClusterRolesPage = () => {
  ...
  const [yamlTabs, setYamlTabs] = useState<ClusterRole[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/clusterroles/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'GET', headers: token ? { Authorization: token } : undefined, signal: controller.signal });
  ...
  const handleOpenYamlEditorFromPanel = (item: ClusterRole) => {
  ...
  const handleSaveYaml = async () => {
    ...
    setYamlSuccessByTab((p) => ({ ...p, [activeYamlTabKey]: null }));
    try {
      const res = await fetch(`/api/clusterroles/${encodeURIComponent(activeYamlItem.name)}/yaml`, { method: 'PUT', headers: { 'Content-Type': 'application/yaml', ...(token ? { Authorization: token } : {}) }, body: yamlContentsByTab[activeYamlTabKey] || '' });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-secondary mt-1">Cluster-wide RBAC roles defining permissions across all namespaces.</p>
      </div>
